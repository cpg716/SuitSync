...
import { useAdminSettings } from '../src/AdminSettingsContext';
...
function GeneralSettingsCard() {
  const { settings, loading, updateSetting, saveSettings, unsavedChanges } = useAdminSettings();

  if (loading) return <Skeleton className="h-48 w-full" />;

  return (
    <Card title="General Settings"
      headerActions={<Button onClick={saveSettings} disabled={!unsavedChanges}>Save Changes</Button>}
    >
      <div className="space-y-4">
        <div className="grid md:grid-cols-2 gap-4">
          <div className="space-y-1">
            <label htmlFor="podium-api-key" className="text-sm font-medium">Podium API Key</label>
            <Input
              id="podium-api-key"
              type="password"
              value={settings.podiumApiKey || ''}
              onChange={(e) => updateSetting('podiumApiKey', e.target.value)}
              placeholder="Enter Podium API Key"
            />
          </div>
          <div className="space-y-1">
            <label htmlFor="sendgrid-api-key" className="text-sm font-medium">SendGrid API Key</label>
            <Input
              id="sendgrid-api-key"
              type="password"
        ...
        <div className="space-y-1">
          <label htmlFor="sendgrid-from" className="text-sm font-medium">SendGrid "From" Email</label>
          <Input
            id="sendgrid-from"
            value={settings.sendgridFrom || ''}
            onChange={(e) => updateSetting('sendgridFrom', e.target.value)}
        ...
        <div className="space-y-1">
          ...
          <Input
            ...
            onChange={(e) => updateSetting('syncFrequency', parseInt(e.target.value, 10))}
            placeholder="e.g., 15"
          />
        </div>
      </div>
...
function UsersAdminCard() {
  ...
  useEffect(() => {
    fetch('/api/users').then(r => r.json()).then(data => {
      // Support both array and {localUsers} shape
      const userArr = Array.isArray(data) ? data : (Array.isArray(data.localUsers) ? data.localUsers : []);
      setUsers(userArr);
      setLoading(false);
    });
  }, []);
  const handleEdit = user => {
    ...
    setEditForm({
      ...
      // Placeholder for per-day schedule
  ...
  const handleScheduleChange = (day, value) => {
    setEditForm(f => ({ ...f, schedule: { ...f.schedule, [day]: value } }));
  };
  const handleSave = async () => {
    // TODO: Wire up backend update
    setEditingUser(null);
  };
  return (
    <Card title="Users (Staff Directory)" className="mb-8">
      {loading ? <Skeleton className="h-24 w-full" /> : (
        <div className="overflow-x-auto">
          <table className="w-full mb-4 text-sm">
            <thead><tr><th>Photo</th><th>Name</th><th>Email</th><th>Phone</th><th>Role</th><th></th></tr></thead>
            <tbody>
              {users.map(u => (
                <tr key={u.id} className="border-b">
                  <td>{u.photoUrl ? <img src={u.photoUrl} alt={u.name} className="h-10 w-10 rounded-full object-cover" /> : <div className="h-10 w-10 rounded-full bg-gray-200" />}</td>
                  <td>{u.name}</td>
                  <td>{u.email}</td>
                  <td>{u.phone || '-'}</td>
                  <td>{u.role}</td>
                  <td><Button variant="outline" onClick={() => handleEdit(u)}>Edit</Button></td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        {editForm && (
          <div className="p-4 space-y-4">
            ...
            <div className="flex gap-4 items-center">
              {editForm.photoUrl ? <img src={editForm.photoUrl} alt={editForm.name} className="h-16 w-16 rounded-full object-cover" /> : <div className="h-16 w-16 rounded-full bg-gray-200" />}
            ...
            <div>
              <label className="block text-sm font-medium">Availability (per day)</label>
              <div className="grid grid-cols-2 gap-2">
                {Object.keys(editForm.schedule).map(day => (
                  <div key={day} className="flex items-center gap-2">
                    <span className="w-10">{day}</span>
                    <Input value={editForm.schedule[day]} onChange={e => handleScheduleChange(day, e.target.value)} placeholder="e.g. 9am-5pm or -" />
                  </div>
                ))}
              </div>
            </div>

...
export default function LightspeedAccountPage() {
  ...
  const linked = user?.lightspeedEmployee;
      ...
      <Card className="w-full max-w-md p-8 bg-white dark:bg-gray-800 shadow-lg border border-accent">
        ...
        {linked ? (
          <div className="flex items-center gap-3 mb-4 p-3 bg-blue-50 dark:bg-blue-900 rounded">
            {linked.photoUrl ? (
              <img src={linked.photoUrl} alt="LS avatar" className="w-12 h-12 rounded-full object-cover" />
            ) : (
              <div className="w-12 h-12 rounded-full bg-blue-200 flex items-center justify-center">
                <span className="text-2xl text-blue-500">LS</span>
              </div>
            )}
            <div>
              <div className="font-semibold text-blue-700 dark:text-blue-200">{linked.name}</div>
              <div className="text-xs text-blue-400">{linked.email}</div>
            </div>
          </div>
        ) : (

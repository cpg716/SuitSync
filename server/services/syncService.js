 ...
 * @param {string} resourceName - The name of the resource (e.g., 'customers', 'products').
 * @param {string} endpoint - The Lightspeed API endpoint (e.g., '/customers').
 * @param {object} model - The Prisma model to upsert into (e.g., prisma.customer).
 * @param {function} mapper - A function to map Lightspeed data to the Prisma model schema.
 */
async function syncResource(resourceName, endpoint, model, mapper) {
  logger.info(`[SyncService] Starting Lightspeed sync for resource: ${resourceName}`);

  try {
    // Immediately set status to SYNCING
    ...
    // The client should be created without a session for system-level background syncs.
    // It will use a persistent token managed by the client itself.
    ...
    // If there were failed items, we still consider the sync a success,
    // but we log an error message that will be visible in the UI.
    ...
    await prisma.syncStatus.update({
      where: { resource: resourceName },
      data: {
        status: 'SUCCESS', // Always set to success, error message shows details
    ...
    // Use upsert to prevent a crash if the status record doesn't exist yet.
    await prisma.syncStatus.upsert({
      where: { resource: resourceName },
      update: {
        status: 'FAILED',
...
const customerMapper = (c) => ({
  name: `${c.first_name || ''} ${c.last_name || ''}`.trim(),
  email: c.contact?.emails?.[0]?.address,
  phone: c.contact?.phones?.[0]?.number,
  lightspeedVersion: BigInt(c.version || 0),
  syncedAt: new Date(),
});

const productMapper = (p) => ({
    name: p.description || p.sku || 'Unnamed Product',
    sku: p.sku,
...
const syncCustomers = () => syncResource('customers', '/customers', prisma.customer, customerMapper);
const syncProducts = () => syncResource('products', '/products', prisma.product, productMapper);

function initScheduledSync() {
  const syncs = [
    { name: 'Customers', func: syncCustomers, interval: 60 * 60 * 1000 }, // every hour
    { name: 'Products', func: syncProducts, interval: 4 * 60 * 60 * 1000 }, // every 4 hours
  ];
...
/**
 * Sync user photos from Lightspeed
 */
async function syncUserPhotos() {
  try {
    logger.info('[SyncService] Starting user photo sync...');
    
    const client = createLightspeedClient(null); // System client
    
    // Get all users with Lightspeed IDs
    const users = await prisma.user.findMany({
      where: {
        lightspeedEmployeeId: { not: null }
      },
      select: {
        id: true,
    ...
    for (const user of users) {
      try {
        // Fetch user details from Lightspeed
        const response = await client.get(`/users/${user.lightspeedEmployeeId}`);
        const lightspeedUser = response.data.data;
        
        // Log available user fields to debug photo issue
        ...
        const newPhotoUrl = lightspeedUser.image_source || 
                           lightspeedUser.image || 
                           lightspeedUser.avatar || 
                           lightspeedUser.photo || 
                           lightspeedUser.profile_image;
        
        logger.info(`[SyncService] User ${user.name} - Current photo: ${user.photoUrl}, New photo: ${newPhotoUrl}`);
        
        // Only update if the photo URL has changed

...
export default function PartyDetail() {
  ...
  return (
    <div className="max-w-3xl mx-auto p-6">
      {/* Sales Flow Progress Bar and Next Action */}
      <div className="mb-6 p-4 bg-primary-light dark:bg-gray-800 rounded-lg flex flex-col gap-2">
        <div className="flex items-center gap-4">
          <span className="font-bold text-lg">Sales Flow Progress</span>
          <span className="ml-auto text-sm text-gray-700 dark:text-gray-300">{nextAction}</span>
        </div>
        <ProgressBar phase={phase} />
        <div className="flex gap-2 mt-2">
          {phase === 0 && <Button className="bg-blue-500 text-white" onClick={() => setShowAppt(true)}>Schedule Suit Selection</Button>}
          {phase === 1 && <Button className="bg-green-500 text-white" onClick={() => setShowAppt(true)}>Schedule Measurements</Button>}
          {phase === 2 && <Button className="bg-yellow-400 text-black" onClick={() => setShowAppt(true)}>Schedule Fitting</Button>}
          {phase === 3 && <Button className="bg-orange-400 text-black" onClick={() => setShowAppt(true)}>Send Pick-Up Reminder</Button>}
        </div>
      </div>
